...
// Fetch first 10 post blur URLs for a specific page (for preview optimization)
...
export const createPost = async (postData) => {
  ...
  // No need to fetch from server anymore!
  ...
  const { pendingFile, isOptimistic, clientId, isUploadingHeic, ...dataToSave } = postData;
...
export const updatePost = async (postId, postData, allPosts) => {
   ...
   * ------------------------------------------- */
  const { isOptimistic, clientId, isUploadingHeic, ...cleanPostData } = postData;
...
export const reindexPages = async (pages) => {
  const confirmedPages = pages
    .filter((p) => !p.isOptimistic)
    .sort((a, b) => (a.order_index || 0) - (b.order_index || 0));
